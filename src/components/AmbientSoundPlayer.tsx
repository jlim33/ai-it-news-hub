...
export function AmbientSoundPlayer({ locale = "ko" }: { locale?: "ko" | "en" }) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "nature":
        return <CloudRain className="w-3.5 h-3.5 text-emerald-400" />;
      default:
  ...
  return (
    <div className="relative">
      ...
      {/* Mini Player Capsule */}
      <div className="flex items-center gap-1.5 p-1.5 px-2.5 rounded-2xl bg-slate-900/90 border border-cyan-500/30 shadow-xs backdrop-blur-md">
        
        {/* Previous Track */}
        <button
          onClick={selectPrevTrack}
          className="p-1 rounded-lg text-slate-400 hover:text-cyan-400 transition-colors hidden sm:block"
        ...
        <button
          ...
          className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all ${
            isPlaying
              ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-sm shadow-cyan-500/25 scale-105"
              : "bg-slate-800 text-slate-300 border border-slate-700 hover:text-cyan-400"
          }`}
          title={isPlaying ? (isEn ? "Pause Audio" : "BGM 일시정지") : (isEn ? "Play Coding Audio" : "코딩 딥워크 BGM 재생")}
        >
          {isLoading ? (
        ...
        >
          ...
          <div className="truncate">
            ...
            <p className="text-[9px] text-cyan-400 font-semibold truncate flex items-center gap-1">
              {isPlaying ? (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  <span>{isEn ? "Continuous Play" : "연속 자동 재생 중"}</span>
                </>
              ) : isEn ? "Coding Radio" : "개발자 힐링 BGM"}
            </p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="absolute right-0 top-12 z-50 w-72 sm:w-84 p-4 rounded-3xl bg-slate-900/95 border border-cyan-500/40 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="font-extrabold text-xs text-white">
                  {isEn ? "Silicon Valley Audio Lounge" : "실리콘밸리 코딩 라운지 BGM"}
                </h4>
                <p className="text-[10px] text-slate-400">
                  {isEn ? "Continuous Lo-Fi, Piano & 528Hz Stream" : "클래식, 샹송, 528Hz 딥포커스 연속 재생"}
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 p-2 rounded-2xl bg-slate-800 border border-slate-700 mb-3">
            ...
            <button
              onClick={togglePlay}
              className="p-1 rounded-lg text-cyan-400 hover:scale-105 transition-transform"
            >
            ...
            <button
              onClick={toggleMute}
              className="p-1 rounded-lg text-slate-400 hover:text-cyan-400 ml-1"
            >
              ...
              return (
                <button
                  ...
                  className={`w-full p-2 rounded-2xl text-left transition-all flex items-center justify-between gap-2 ${
                    isSelected
                      ? "bg-cyan-950/60 border border-cyan-500/60 text-cyan-200 shadow-xs"
